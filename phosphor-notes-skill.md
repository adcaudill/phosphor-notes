...
| `add_task` | Adding one checkbox task. Defaults to today's daily note if you omit `file` (auto-creating it if needed) - this is almost always what "add a task for today" or "remind me to X" means. Pass `due`/`recurrence` as their own parameters (`YYYY-MM-DD` / `+1d`, `+2w`, `+1m`, `+1y`) rather than typing the 📅/🔁 emoji yourself into `text`. To add a task under a specific existing bullet rather than at the end of the note, build the task line yourself and use `insert_under_bullet` instead. |
